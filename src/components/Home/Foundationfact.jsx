...
export default function Foundationfact() {
  
  // Detect when section is in view
  const { ref, inView } = useInView({
    triggerOnce: false, // ❌ allow multiple triggers
    threshold: 0.5, // ✅ triggers when ~50% visible (center area)
  });

  return (
    <section ref={ref} className="py-16 px-6 text-center bg-[#fdf9e9]">
      {/* Heading */}
      <h2 className="text-4xl sm:text-5xl lg:text-6xl font-serif text-red-950 mb-4 tracking-widest">
        Foundation facts
      </h2>
      <p className="text-red-950 mb-12 text-base sm:text-lg">
      ...
      <div className="hidden lg:grid grid-cols-5 gap-10 max-w-6xl mx-auto">
        {stats.map((stat, index) => (
          <div key={index} className="flex flex-col items-center">
            ...
            <p className="text-red-950 mt-4 font-semibold text-lg">
              {stat.label}
            </p>
          </div>
        ))}
